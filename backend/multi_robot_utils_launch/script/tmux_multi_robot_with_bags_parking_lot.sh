...
BAG_PLAY_RATE=2.0
BAG_DIR="${SLIDE_SLAM_BAG_DIR:-/opt/bags/vems-slam-bags/all_slide_slam_public_demos/parking-lot}"
...
tmux split-window -h -t $SESSION_NAME
tmux select-pane -t $SESSION_NAME:1.0
tmux send-keys -t $SESSION_NAME "$SETUP_ROS_STRING; sleep 20; cd $BAG_DIR && ros2 bag play robot0* -r $BAG_PLAY_RATE --start-offset 60 --topics /Odometry /robot0/semantic_meas_sync_odom --remap /Odometry:=/robot0/odom" Enter
tmux select-pane -t $SESSION_NAME:1.1
tmux send-keys -t $SESSION_NAME "$SETUP_ROS_STRING; sleep 20; cd $BAG_DIR && ros2 bag play robot1* -r $BAG_PLAY_RATE --start-offset 60 --topics /Odometry /robot0/semantic_meas_sync_odom --remap /Odometry:=/robot1/odom /robot0/semantic_meas_sync_odom:=/robot1/semantic_meas_sync_odom" Enter
tmux select-pane -t $SESSION_NAME:1.2
tmux send-keys -t $SESSION_NAME "$SETUP_ROS_STRING; sleep 2; cd $BAG_DIR && ros2 bag play robot2* -r $BAG_PLAY_RATE --topics /Odometry /robot0/semantic_meas_sync_odom --remap /Odometry:=/robot2/odom /robot0/semantic_meas_sync_odom:=/robot2/semantic_meas_sync_odom" Enter
tmux select-pane -t $SESSION_NAME:1.3
